...
if(isset($_POST['name']) && isset($_POST['rollnumber']) && isset($_POST['password']) && isset($_POST['cpassword'])){
		...
		if(empty($name)){
			...
			if(mysqli_num_rows($status) >=1){
				...
				if($pass === $cpass){
				$insert = "INSERT INTO students(username,name,passwords,cpassword,phonenumber,email) VALUES('$roll','$name','$pass','$cpass','$phonenumber','$email')";
				// Function call
				//function_alert("Account Successfully Created..!");
					if(mysqli_query($conn,$insert)){
						header("Location: index.php?success=Account Created Successfully");
					}else{
						header("Location: index.php?error=Error in connecting DataBase");
					}
				}else{
					header("Location: index.php?error=password and Confirm password should be same");
					exit();	
				}
